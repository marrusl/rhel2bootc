...
def _write_config_tree(snapshot: InspectionSnapshot, output_dir: Path) -> None:
    ...
    if ug:
        ...
        sysusers_users = [u for u in (ug.users or []) if u.get("strategy") == "sysusers"]
        sysusers_groups = [g for g in (ug.groups or []) if g.get("strategy") == "sysusers"]
        ...
        exact_users = {u.get("name") for u in (ug.users or []) if u.get("strategy") == "exact-copy"}
        exact_groups = {g.get("name") for g in (ug.groups or []) if g.get("strategy") == "exact-copy"}
        ...
        blueprint_users = [u for u in (ug.users or []) if u.get("strategy") == "blueprint"]
        blueprint_groups = [g for g in (ug.groups or []) if g.get("strategy") == "blueprint"]
...
def _render_containerfile_content(snapshot: InspectionSnapshot, output_dir: Path) -> str:
    ...
    if ug and ug.users:
        ...
        sysusers_users = [u for u in ug.users if u.get("strategy") == "sysusers"]
        useradd_users = [u for u in ug.users if u.get("strategy") == "useradd"]
        exact_users = [u for u in ug.users if u.get("strategy") == "exact-copy"]
        blueprint_users = [u for u in ug.users if u.get("strategy") == "blueprint"]
        kickstart_users = [u for u in ug.users if u.get("strategy") == "kickstart"]
        ...
        # --- useradd strategy ---
        if useradd_users:
            useradd_groups = [g for g in (ug.groups or []) if g.get("strategy") == "useradd"]
